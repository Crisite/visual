...
import pandas as pd
from pyecharts.charts import Timeline, Pie, Map, Line, Bar
...
unique_epidemic_data["provinceName"] = unique_epidemic_data.provinceName.apply(lambda province_name: province_name.replace('省', ''))
unique_epidemic_data["provinceName"] = unique_epidemic_data.provinceName.apply(lambda province_name: province_name.replace('自治区', ''))
unique_epidemic_data["provinceName"] = unique_epidemic_data.provinceName.apply(lambda province_name: province_name.replace('市', ''))
...
epidemic_trend_line = (
    ...
    .set_global_opts(
        title_opts=opts.TitleOpts(title='疫情走势图',subtitle=''),
        tooltip_opts=opts.TooltipOpts(trigger='axis'),
        toolbox_opts=opts.ToolboxOpts(is_show=False),
        xaxis_opts=opts.AxisOpts(type_='category',boundary_gap=False)
    )
    .render('./reasult/epidemic_trend_line.html')
)
...
for updateDate in updateDates:
    ...
    one_day_bar = (
        ...
        .set_global_opts(title_opts=opts.TitleOpts('{}top5-city'.format(updateDate)))
    )
    epidemic_percent_time.add(one_day_bar, updateDate)
epidemic_percent_time.render('./reasult/epidemic_top5_bar.html')
